...
class TestCycleRepository {
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        TestCycle(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
    ...
    await TestCycle(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await TestCycle(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async filterIdsInTenant(
    ...
    const records = await TestCycle(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      TestCycle(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    if (countOnly) {
      const count = await TestCycle(
        options.database,
      ).countDocuments(criteria);
      return { count };
    }

    let rows = await TestCycle(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: TestCycle(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
...
export default TestCycleRepository;
/// File is generated from https://studio.fabbuilder.com - testCycle
