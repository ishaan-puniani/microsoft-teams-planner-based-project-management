...
class TestCaseRepository {
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        TestCase(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
    ...
    await TestCase(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await TestCase(options.database).deleteOne(
      { _id: id },
      options,
    );
  ...
  static async filterIdsInTenant(
    ...
    const records = await TestCase(options.database)
      .find({
        _id: { $in: ids },
        tenant: currentTenant.id,
      })
  ...
  static async count(filter, options: IRepositoryOptions) {
    ...
    return MongooseRepository.wrapWithSessionIfExists(
      TestCase(options.database).countDocuments({
        ...filter,
        tenant: currentTenant.id,
      }),
      options,
  ...
  static async findAndCountAll(
    ...
    let rows = await TestCase(options.database)
      .find(criteria)
      .skip(skip)
      .limit(limitEscaped)
      .sort(sort)
  ...
  static async _createAuditLog(
    ...
    await AuditLogRepository.log(
      {
        entityName: TestCase(options.database).modelName,
        entityId: id,
        action,
        values: data,
      },
