...
const ScheduledEventViewItem = (props) => {
  ...
  const displayableRecord = (record) => {
    if (hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link className="btn btn-link" to={`/scheduled-event/${record.id}`}>
            {record.title}
          </Link>
        </div>
      );
  ...
  if (!valueAsArray().length) return null;
...
ScheduledEventViewItem.propTypes = {
  label: PropTypes.string,
  value: PropTypes.any,
};

export default ScheduledEventViewItem;
