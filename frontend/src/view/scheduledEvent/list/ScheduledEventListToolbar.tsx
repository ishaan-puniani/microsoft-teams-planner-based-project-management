...
const ScheduledEventListToolbar = (props) => {
  ...
  const renderExportButton = () => {
    ...
    const button = (
      <button className="btn btn-light" disabled={disabled} onClick={doExport} type="button">
        <ButtonIcon loading={exportLoading} iconClass="far fa-file-excel" />{' '}
        {i18n('common.export')}
      </button>
    );
    if (disabled) {
      return (
        <span
          ...
          data-for="scheduledEvent-list-toolbar-export-tooltip"
        >
          {button}
          <Tooltip id="scheduledEvent-list-toolbar-export-tooltip" />
        </span>
      );
    }
    return button;
  ...
  const renderDestroyButton = () => {
    ...
    const button = (
      <button
        ...
        type="button"
        onClick={() => setDestroyAllConfirmVisible(true)}
      >
        <ButtonIcon loading={destroyLoading} iconClass="far fa-trash-alt" />{' '}
        {i18n('common.destroy')}
      </button>
    );
    if (disabled) {
      return (
        <span
          ...
          data-tip-disable={!disabled}
          data-for="scheduledEvent-list-toolbar-destroy-tooltip"
        >
          {button}
          <Tooltip id="scheduledEvent-list-toolbar-destroy-tooltip" />
        </span>
      );
    }
    return button;
  ...
  return (
    <Toolbar>
      ...
      {hasPermissionToImport && (
        <Link to="/scheduled-event/importer">
          <button className="btn btn-primary" type="button">
      ...
      {hasPermissionToAuditLogs && (
        <Link to="/audit-logs?entityNames=scheduledEvent">
      ...
      {destroyAllConfirmVisible && (
        <ConfirmModal
          ...
          onClose={() => setDestroyAllConfirmVisible(false)}
...
export default ScheduledEventListToolbar;
